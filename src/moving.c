...
void lmove(void) {
	...
	if (inorbit) {
		printDialog(SULU, " \"Leaving standard orbit.\"");
...
void dock(void) {
	...
	if (stdamtim != 1e30 &&
		...
		printDialog(UHURA, " \"Captain, an important message from the starbase:\"");
...
static void getcd(int isprobe, int akey) {
	...
	if (landed == 1 && !isprobe) {
		prout("You can't leave standard orbit until you");
	...
	if (automatic) {
		...
		if (!isprobe) {
			if (itemp) {
				if (iprompt) {
					printDialog(SULU, " \"Course locked in for");
					// clprintf("%sHelmsman Sulu%s- \"Course locked in for", clrbrblue, clrdefault);
					cramlc(2, irows, icols);
					prout(".\"");
				}
			}
			else printDialog(CHEKOV, " \"Course laid in, Captain.\"");
			// else clprintf("%sEnsign Chekov%s- \"Course laid in, Captain.\"", clrbrblue, clrdefault);
		}
	...
	if (itemp == 2 && !isprobe) {
		skip(1);
		printDialog(SULU, " \"Aye, Sir.\"");
		// clprintf("%sHelmsman Sulu%s- \"Aye, Sir.\"", clrbrblue, clrdefault);
	}
...
void impuls(void) {
	...
	if (damage[DIMPULS]) {
		chew();
		skip(1);
		printDialog(SCOTTY, " \"The impulse engines are damaged, Sir.\"");
		// clprintf("%sEngineer Scott%s- \"The impulse engines are damaged, Sir.\"", clrbrblue, clrdefault);
		return;
	}
	...
	if (power >= energy) {
		...
		skip(1);
		printDialog(SPOCK," \"Captain, the impulse engines");
		// clprintf("%sSpock%s- \"Captain, the impulse engines", clrbrblue, clrdefault);
	...
	Time = dist/0.095;
	if (Time >= d.remtime) {
		printDialog(SPOCK," \"Captain, our speed under impulse");
		// clprintf("%sSpock%s- \"Captain, our speed under impulse", clrbrblue, clrdefault);
...
void warp(int i) {
	...
	if (i!=2) { /* Not WARPX entry */
		...
		if (iscloaked) {
			chew();
			skip(1);
			printDialog(SCOTTY," \"The warp engines cannot be used while cloaked, Sir.\"");
			// clprintf("%sEngineer Scott%s- \"The warp engines cannot be used while cloaked, Sir.\"", clrbrblue, clrdefault);
			return;
		}
#endif
		if (damage[DWARPEN] > 10.0) {
			chew();
			skip(1);
			printDialog(SCOTTY," \"The warp engines are damaged, Sir.\"");
			// clprintf("%sEngineer Scott%s- \"The warp engines are damaged, Sir.\"", clrbrblue, clrdefault); // Was "Impulse" 10/2013
			return;
		}
		if (damage[DWARPEN] > 0.0 && warpfac > 4.0) {
			chew();
			skip(1);
			printDialog(SCOTTY," \"Sorry, Captain. Until this damage");
			// clprintf("%sEngineer Scott%s- \"Sorry, Captain. Until this damage", clrbrblue, clrdefault);
		...
		if (Time >= 0.8*d.remtime) {
			skip(1);
			printDialog(SPOCK," \"Captain, I compute that such");
			// clprintf("%sSpock%s- \"Captain, I compute that such", clrbrblue, clrdefault);
...
void setwrp(void) {
	...
	if (damage[DWARPEN] > 0.0 && aaitem > 4.0) {
		printDialog(SCOTTY, " \"I'm doing my best, Captain,\n  but right now we can only go warp 4.\"");
		// prout("Engineer Scott- \"I'm doing my best, Captain,\n"
		// 	  "  but right now we can only go warp 4.\"");
		return;
	}
	if (aaitem > 10.0) {
		printDialog(SULU, " \"Our top speed is warp 10, Captain.\"");
		// prout("Helmsman Sulu- \"Our top speed is warp 10, Captain.\"");
		return;
	}
	if (aaitem < 1.0) {
		printDialog(SULU, " \"We can't go below warp 1, Captain.\"");
	...
	if (warpfac < 8.00) {
		printDialog(SCOTTY, " \"Aye, but our maximum safe speed is warp 6.\"");
		return;
	}
	if (warpfac == 10.0) {
		printDialog(SCOTTY, " \"Aye, Captain, we'll try it.\"");
		return;
	}
	printDialog(SCOTTY, " \"Aye, Captain, but our engines may not take it.\"");
	return;
}
...
void timwrp() {
	...
	if (d.snap && Rand() < 0.5) {
		...
		for (l = 1; l <= inplan; l++) {
			if (d.plnets[l].known == 2) {
				...
				if (iscraft==1 && ship==IHE) {
					printDialog(CHEKOV, " \"Security reports the Galileo has disappeared, Sir!");
					iscraft = 0;
				}
			}
		}
		...
		if (iscraft==0 && gotit==0 && damage[DSHUTTL] >= 0.0) {
			printDialog(CHEKOV, " \"Security reports the Galileo has reappeared in the dock!\"");
			iscraft = 1;
		}
...
void probe(void) {
	...
	if (nprobes == 0) {
		...
		skip(1);
		if (ship == IHE) 
			printDialog(SCOTTY, " \"We have no more deep space probes, Sir.\"");
		else
	...
	if (damage[DDSP] != 0.0) {
		chew();
		skip(1);
		printDialog(SCOTTY, " \"The probe launcher is damaged, Sir.\"");
		return;
	}
	if (future[FDSPROB] != 1e30) {
		...
		skip(1);
		if (REPORTS) {
			printDialog(UHURA, " \"The previous probe is still reporting data, Sir.\"");
		} else {
			printDialog(SPOCK, " \"Records show the previous probe has not yet");
	...
	printDialog(CHEKOV, " \"The deep space probe is launched, Captain.\"");
	return;
}
...
void help(void) {
	...
	if (condit == IHDOCKED) {
		printDialog(UHURA, " \"But Captain, we're already docked.\"");
		return;
	}
	if (damage[DRADIO] != 0) {
	...
	if (d.rembase==0) {
		printDialog(UHURA, " \"Captain, I'm not getting any response from Starbase.\"");
		return;
	}
	if (landed == 1) {
	...
	for (l = 1; l <= 5; l++) {
		...
		if (ix>=1 && ix<=10 && iy>=1 && iy<=10 && quad[ix][iy]==IHDOT) {
			...
			dock();
			skip(1);
			printDialog(UHURA, " \"Captain, we made it!\"");
			return;
		}
	}
	finish(FMATERIALIZE);
