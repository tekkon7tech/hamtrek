...
void preport(void) {
	...
	skip(1);
	chew();
	printDialog(SPOCK, " \"Planet report follows, Captain.\"");
	skip(1);
	for (i = 1; i <= inplan; i++) {
		if (d.plnets[i].known
#ifdef DEBUG
...
void orbit(void) {
	...
	Time = 0.02+0.03*Rand();
	printDialog(SULU, " \"Entering standard orbit, Sir.\"");
	newcnd();
	if (consumeTime()) return;
	printDialog(SULU, " \"Entered orbit at altitude ");
...
void sensor(void) {
	...
	printDialog(SPOCK, " \"Sensor scan for");
...
void beam(void) {
	...
	if (damage[DTRANSP] != 0) {
		...
		if (damage[DSHUTTL]==0 && (d.plnets[iplnet].known==2 || iscraft == 1)) {
			skip(1);
			printDialog(SPOCK, " \"May I suggest the shuttle craft, Sir?\" ");
			if (ja() != 0) shuttle();
		}
		return;
	}
	...
	if (d.plnets[iplnet].known==0) {
		printDialog(SPOCK, " \"Captain, we have no information on this planet");
	...
	if (landed==1) {
		/* Coming from planet */
		if (d.plnets[iplnet].known==2) {
			printDialog(SPOCK, " \"Wouldn't you rather take the Galileo?\" ");
			if (ja() != 0) {
				chew();
				return;
			}
		...
		if (d.plnets[iplnet].crystals==0) {
			printDialog(SPOCK, " \"Captain, I fail to see the logic in");
		...
		printDialog(SCOTTY, " \"Transporter room ready, Sir.\"");
		skip(1);
	...
	if (Rand() > 0.98) {
		...
		skip(2);
		printDialog(SCOTTY, " \"Oh my God!  I've lost them.\"");
		finish(FLOST);
		return;
	}
...
void usecrystals(void) {
	...
	if (energy >= 1000) {
		printDialog(SPOCK, " \"Captain, Starfleet Regulations prohibit such an operation");
		prout("  except when condition Yellow exists.");
		return;
	}
	printDialog(SPOCK, " \"Captain, I must warn you that loading");
	...
	skip(1);
	printDialog(SCOTTY, " \"(GULP) Aye Sir.");
	prout("  Mr. Spock and I will try it.\"");
	skip(1);
	printDialog(SPOCK, " \"Crystals in place, Sir.");
	prout("  Ready to activate circuit.\"");
	skip(1);
	printDialog(SCOTTY, " \"Keep your fingers crossed, Sir!\"");
	skip(1);
	if (Rand() <= cryprob) {
...
void shuttle(void) {
	...
	if (landed == 1) {
		...
		if (iscraft==1) {
			/* Galileo on ship! */
			if (damage[DTRANSP]==0) {
				printDialog(SPOCK, " \"Would you rather use the transporter?\" ");
				if (ja() != 0) {
					beam();
					return;
				}
...
void deathray(void) {
	...
	if (nenhere==0) {
		printDialog(SULU, " \"But Sir, there are no enemies in this quadrant.\"");
		return;
	}
	if (damage[DDRAY] > 0.0) {
	...
	printDialog(SPOCK, " \"Captain, the 'Experimental Death Ray'");
	prout("  is highly unpredictable.  Considering the alternatives,");
	prout("  are you sure this is wise?\" ");
	if (ja()==0) return;
	printDialog(SPOCK, " \"Acknowledged.\"");
	skip(1);
	ididit=1;
	...
	skip(1);
	printDialog(SPOCK, " \"Preparations complete, sir.\"");
	prout("Kirk-  \"Engage!\"");
	skip(1);
	prouts("WHIRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR");
	skip(1);
	if (r > .30) {
		printDialog(SULU, " \"Captain!  It's working!\"");
		skip(2);
		while (nenhere > 0) {
			deadkl(kx[1],ky[1],quad[kx[1]][ky[1]],kx[1],ky[1]);
		}
		printDialog(CHEKOV, " \"Congratulations, Captain!\"");
		if (d.remkl == 0) finish(FWON);
		printDialog(SPOCK, " \"Captain, I believe the `Experimental Death Ray'");
		if (Rand() <= 0.05) {
	...
	if (r <= .55) {
		printDialog(SULU, " \"Captain!  Yagabandaghangrapl, brachriigringlanbla!\"");
		skip(1);
		printDialog(UHURA, " \"Graaeek!  Graaeek!\"");
		skip(1);
		printDialog(SPOCK, " \"Fascinating!  . . . All humans aboard");
	...
	if (r <= 0.75) {
		int i,j;
		printDialog(SULU, " \"Captain!  It's   --WHAT?!?!\"");
		skip(2);
		printDialog(SPOCK, " \"I believe the word is");
		prouts(" *ASTONISHING*");
		printDialog(SULU, " .");
		for (i=1; i<=10; i++)
			for (j=1; j<=10; j++)
	...
	printDialog(SULU, " \"Captain!  The Death Ray is creating tribbles!\"");
	skip(1);
	printDialog(SCOTTY, " \"There are so many tribbles down here");
