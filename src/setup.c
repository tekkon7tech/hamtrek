...
void prelim(void) {
	...
	skip(1);
	SetColor(clrbrblue);
	prout("             _____  __  __ ___  ______ ___");
	prout("            / ___/ / / / / __ \\/ ____/ __ \\");
	SetColor(clrcyan);
	prout("            \\__ \\ / / / / /_/ / __/ / /_/ /");
	prout("            ___/ / /_/ / ____/ /___/ _, _/");
	SetColor(clrbrwhite);
	prout("           /____/\\___/_/___/_____/_/ |_|");
	SetColor(clrbrblue);
	prout("    __  _ ___    ___ ___   ______ ___  ______ _ __");
	prout("   / / / /   |  /  |/  /  /_  __/ __ \\/ ____/ //_/");
	SetColor(clrcyan);
	prout("  / /_/ / /| | / /|_/ /    / / / /_/ / __/ / ,<");
	prout(" / __  / ___ |/ /  / /    / / / _, _/ /___/ /| |");
	SetColor(clrbrwhite);
	prout("/_/ /_/_/  |_/_/  /_/    /_/ /_/ |_/_____/_/ |_|");
	DefaultGameColor();
	skip(1);
}
...
void abandn(void) {
	...
	if (condit==IHDOCKED) {
		...
		if (damage[DSHUTTL]<0) {
			prout("Shuttle craft now serving Klingon Gagh.");
			return;
		}
		if (damage[DSHUTTL]>0) {
...
//	idate = date;
	...
	{
		clprintf("%d Stardates remaining...\n\n", (int)intime);

		printDialog(UHURA, " \"Intelligence report from Starfleet Command, Captian.\"\n");
		clprintf("  %s(%s%3d%s)%s Klingons%s\n", clrbrwhite, clrbrred, inkling, clrbrwhite, clrbrred, clrdefault);
		clprintf("  %s(%s???%s)%s Romulans%s\n", clrbrwhite, clrbrgreen, clrbrwhite, clrbrgreen, clrdefault);
		clprintf("  %s(%s%3d%s)%s Starbases%s at locations: ", clrbrwhite, clrbrcyan, inbase, clrbrwhite, clrbrcyan, clrdefault);
		// printf("Stardate %d.\n\n"
		// 	   "%3d Klingons,\nan unknown number of Romulans\n",
		// 	   (int)d.date, inkling);
		// printf("%3d stardates\n%3d starbases in  ",(int)intime, inbase);
		if (d.nscrem) clprintf("\n%sALERT%s: long-range sensors have detected a Super-Commander.", clrbrred, clrdefault);
	}
	for (i = 1; i <= inbase; i++) {
...
int choose(void) {
	...
	while (TRUE) {
		if (fromcommandline) /* Can start with command line options */
			...
			clprintf("Game type: %s[%sREGULAR%s]%s, %s[%sTOURNAMENT%s]%s, %s[%sFROZEN%s]%s ? ",
				clrbrwhite,
				clrbrblue,
				clrbrwhite,
				clrdefault,
		...
		proutn(citem);
		prout("\"?");
		// for(int x=0;x<23;x++) printf("%d", (int)citem[x]);
		chew();
	}
	while (length==0 || skill==0) {
		if (scan() == IHALPHA) {
			...
			if (length==0)
			{
				// proutn("Would you like a Short, Medium, or Long game? ");
				clprintf("Game length: %s[%sSHORT%s]%s, %s[%sMEDIUM%s]%s, %s[%sLONG%s]%s ? ",
					clrbrwhite,
					clrbrblue,
					clrbrwhite,
					clrdefault,
			...
			else if (skill == 0)
			{
				...
				clprintf("\nGame skill: %s[%sNOVICE%s]%s, %s[%sFAIR%s]%s, %s[%sEXPERT%s]%s, %s[%sEMERITUS%s]%s ? ",
					clrbrwhite,
					clrbrblue,
					clrbrwhite,
					clrdefault,
	...
	while (TRUE) {
		...
		chew();
		if (*passwd != 0) break;
		clprintf("\nPlease type in a secret password %s(%s9 characters maximum%s)%s: ",
			clrbrwhite,
			clrbrblue,
			clrbrwhite,
			clrdefault);
...
void newqad(int shutup) {
	...
	if (irhere > 0 && klhere == 0 && basex == 0) {
		...
		if (REPORTS) { 
			skip(1);
			printDialog(UHURA, " \"Captain, an urgent message.");
	...
	if (shutup==0) {
		...
		if (thingx == quadx && thingy == quady) {
			...
			if (damage[DSRSENS] == 0.0) {
				skip(1);
				printDialog(SPOCK, " \"Captain, this is most unusual.");
