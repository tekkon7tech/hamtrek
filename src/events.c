...
void events(void) {
	...
	for (;;) {
		...
		if (stdamtim != 1e30 && REPORTS) {
			stdamtim = 1e30;
			printDialog(UHURA, " \"Captain, the sub-space radio is working and");
		...
		switch (line) {
			...
			case FBATTAK: /* Commander attacks starbase */
				...
				ipage = 1;
				skip(1);
				printDialog(UHURA, " \"Captain, the starbase in");
				cramlc(1, batx, baty);
				skip(1);
			...
			case FCDBAS: /* Commander succeeds in destroying base */
				...
				if (batx==quadx && baty==quady) {
					...
					ipage = 1;
					skip(1);
					printDialog(UHURA, " \"Captain, Starfleet Command reports that");
				...
				if (probecx != i || probecy != j) {
					...
					if (i < 1 || i > 8 || j < 1 || j > 8 ||
						...
						if (REPORTS) {
							...
							ipage = 1;
							skip(1);
							printDialog(UHURA, " \"The deep space probe ");
							if (i < 1 ||i > 8 || j < 1 || j > 8)
					...
					if (REPORTS) {
						...
						ipage = 1;
						skip(1);
						printDialog(UHURA, " \"The deep space probe is now in ");
